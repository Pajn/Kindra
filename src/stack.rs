...
fn topologically_sort_edited_graph(
    ...
    while let Some(branch) = ready.first().cloned() {
        ready.remove(0);
        ...
        if let Some(child_names) = children.get(&branch) {
            ...
            ready.sort_by_key(|candidate| order_hint.get(candidate).copied().unwrap_or(usize::MAX));
        }
    }
...
fn range_changes_present_in_target(
    ...
    let mut compare = Command::new("git");
    compare
        .arg("diff")
        .arg("--quiet")
        .arg("--no-ext-diff")
    ...
    match status.code() {
        ...
        Some(1) => Ok(false),
        _ => Err(anyhow!(
            "git diff failed while comparing branch changes against target content."
        )),
    }
}
...
fn ordered_stack_lineage(
    ...
    let mut lineage = stack_branches
        .iter()
        .filter(|branch| {
            branch.id == top_id || repo.graph_descendant_of(top_id, branch.id).unwrap_or(false)
        })
        .cloned()
        .collect::<Vec<_>>();
