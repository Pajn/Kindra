...
fn test_tree_pr_tolerates_team_review_requests() {
    ...
    std::fs::write(
        ...
        r#"#!/bin/bash
if [[ "$1" == "pr" ]] && [[ "$2" == "list" ]]; then
    echo '[{"number":10,"headRefName":"feature-a","baseRefName":"main","isDraft":false,"author":{"login":"me"},"reviewRequests":[{"requestedReviewer":{"name":"platform-team","slug":"platform-team"}}]},{"number":11,"headRefName":"feature-b","baseRefName":"feature-a","isDraft":false,"author":{"login":"me"},"reviewRequests":[{"requestedReviewer":{"login":"alice"}}]}]'
    exit 0
fi
echo "unexpected gh command: $@" >&2
exit 1
"#,
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("#10") && stdout.contains("#11"),
        "both PRs should render despite a team review request, got:\n{}",
        stdout
    );
}
